...
// =============================================================================
// Module  : conv_layer
// Function: 4-channel parallel convolution layer wrapper.
//           Each channel is an independent PE instance sharing the same
//           control signals (conv_mode / conv_in_valid).
//           Weights are pre-packed externally as [ch3|ch2|ch1|ch0].
//           Global output valids are driven by AND-reduction of all channels.
// =============================================================================
module conv_layer #(
    parameter DIN_WIDTH     = 8,   // Input pixel bit-width
    parameter NUM           = 9,   // Kernel elements (3×3)
    parameter WEIGHT_WIDTH  = 4,   // Weight bit-width
    parameter MUL_WIDTH     = 12,  // Multiplier output width
    parameter DOUT_WIDTH_1D = 14,  // 1-D partial sum width
    parameter DOUT_WIDTH_2D = 16   // 2-D accumulated output width
) (
    input  wire                                   clk,
    input  wire                                   rst_n,

    // ---- 4-channel 3×3 pixel windows (flattened, NUM pixels each) ----------
    ...
    // ---- Packed weight bus: [ch3|ch2|ch1|ch0], NUM*WEIGHT_WIDTH bits each --
    input  wire signed [WEIGHT_WIDTH*NUM*4-1:0]   weight,

    // ---- Shared control -----------------------------------------------------
    input  wire                                   conv_mode,       // 1-D / 2-D select
    input  wire                                   conv_in_valid,   // input data valid

    // ---- 2-D convolution results per channel --------------------------------
    ...
    // ---- 1-D partial sums for all 4 channels packed ------------------------
    output wire signed [DOUT_WIDTH_1D*3*4-1:0]    conv1D_dout,     // 3 row-sums × 4 channels

    // ---- Global output valid flags ------------------------------------------
...
// Per-channel valid flags — AND-reduced to produce global valid strobes
...
// =============================================================================
// 4-channel PE array: one Processing Element instance per channel
// =============================================================================
generate
    genvar i;
